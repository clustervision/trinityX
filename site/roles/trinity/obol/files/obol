...
def exists(key, value):
    try:
        if key == 'uid':
            ...
            return grp.getgrgid(int(value.decode())).gr_gid
        elif key == 'groupname':
...
def user_add(b, username, cn, sn, givenName, password, uid, gid, mail, phone,
    ...
    add_record = [
     ...
     ('shadowLastChange', [b'%d' % (time.time() / 86400)])
    ]

    if givenName:
        add_record.append(('givenName', [givenName.encode('utf-8')]))

    if mail:
        add_record.append(('mail', [mail.encode('utf-8')]))
    ...
    if exists('gid', gidNumber) is False:
        ...
        add_record = [
            ...
            ('member', [str('uid=%s,ou=People,%s' % (username, b)).encode('utf-8')]),
            ('gidNumber', [gidNumber])
        ]
...
def group_list(b):
    ...
    for dn, attrs in conn.search_s(base_dn, ldap.SCOPE_SUBTREE, filter):
        print(attrs['gidNumber'][0].decode(), attrs['cn'][0].decode())
    ...
    for name in username:
        try:
            ...
            mod_attrs.append((ldap.MOD_ADD, 'member',
                              str('uid=%s,ou=People,%s' % (name, b)).encode('utf-8')))
            conn.modify_s(dn, mod_attrs)
        except Exception as error:
...
def increment_id(b, id_type):
    ...
    mod_attrs = [(ldap.MOD_REPLACE, 'uidNumber', str(int(uidNumber) + 1).encode('utf-8'))]
    conn.modify_s(dn, mod_attrs)
