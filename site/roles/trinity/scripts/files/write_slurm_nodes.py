...
COMMENT_CHAR = '#'
CONFIG_FILEPATH = "/etc/slurm/slurm-nodes.conf"
...
class ConfigFile:
    ...
    def parse_groups(self):
        ...
        for line in self.managed_lines:
            if line.startswith(COMMENT_CHAR):
                if line.startswith(self._group_comment()):
                    current_groupname = line.split(":")[1].strip()
                    groups[current_groupname] = []
            else:
                nodename = line.split("=")[1].strip()
                groups[current_groupname].append(nodename)
    ...
    def dump_groups(self):
        ...
        managed_lines = []
        for groupname in self.groups:
            managed_lines.append(f"\n{self._group_comment(groupname)}\n")
            for nodename in self.groups[groupname]:
                managed_lines.append(f"NodeName={nodename}\n")
