...
COMMENT_CHAR = '#'
CONFIG_FILEPATH = "/etc/slurm/slurm-partitions.conf"
    ...
    def parse_groups(self):
        ...
        for line in self.managed_lines:
            if not (line.startswith(COMMENT_CHAR) or line.strip() == ''):
                line_regex = r"PartitionName=(?P<groupname>\w+)\s+Nodes=(?P<nodenames>[\w,]+)"
                groupname, _nodenames = re.search(line_regex, line).groups()
                if groupname == 'defq':
                    continue
                nodenames = _nodenames.split(',')
